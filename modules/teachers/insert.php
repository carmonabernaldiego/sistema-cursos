...
if ($result = $conexion->query($sql)) {
	if ($row = mysqli_fetch_array($result)) {
		...
		if (mysqli_query($conexion, $sql_insert_user)) {
			$sql_insert_teacher = "INSERT INTO teachers(user, name, surnames, curp, rfc, gender, date_of_birth, phone, address, level_studies, specialty, career, created_at) VALUES('" . trim($_POST['txtuserid']) . "', '" . trim($_POST['txtname']) . "', '" . trim($_POST['txtsurnames']) . "', '" . trim($_POST['txtcurp']) . "', '" . trim($_POST['txtrfc']) . "', '" . trim($_POST['selectgender']) . "', '" . trim($_POST['dateofbirth']) . "', '" . trim($_POST['txtphone']) . "', '" . trim($_POST['txtaddress']) . "', '" . trim($_POST['selectlevelstudies']) . "', '" . trim($_POST['txtspecialty']) . "', '" . $$_POST["selectCareer"] . "', '" . $date . "')";

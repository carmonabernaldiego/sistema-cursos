...
if ($result = $conexion->query($sql)) {
	if ($row = mysqli_fetch_array($result)) {
		...
		if (mysqli_query($conexion, $sql_insert_user)) {
			$sql_insert_administrative = "INSERT INTO administratives(user, name, surnames, curp, rfc, date_of_birth, gender, phone, address, level_studies, occupation, observations, created_at) VALUES('" . $user . "', '" . trim($_POST['txtname']) . "', '" . trim($_POST['txtsurnames']) . "', '" . trim($_POST['txtcurp']) . "', '" . trim($_POST['txtrfc']) . "', '" . trim($_POST['dateofbirth']) . "', '" . trim($_POST['selectgender']) . "', '" . trim($_POST['txtphone']) . "', '" . trim($_POST['txtaddress']) . "', '" . trim($_POST['selectlevelstudies']) . "', '" . trim($_POST['txtoccupation']) . "', '" . trim($_POST['txtobservation']) . "', '" . $date . "')";
